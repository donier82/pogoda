...
async def get_pogoda():
    ...
    # response = requests.get('https://api.openweathermap.org/data/2.5/weather?q=Osh&appid=7ae9895ce71d3ad63f3c713fd0e2bab9&units=metric')
    #temp = response.get("temp").json()
    # if temp:
    #     return f'Стоимость биткоина USDT на {time.ctime()}, {temp}'
    # else:
    #     return f'Не удалось получить цену биткоина'
    res=requests.get('https://api.openweathermap.org/data/2.5/weather?q=Osh&appid=7ae9895ce71d3ad63f3c713fd0e2bab9&units=metric')
    temp=json.load(res.text)
    return temp
...
async def on():
    await bot.set_my_commands([
        ...
        BotCommand(command="/pogoda", description='Start USDT monitoring'),        
